 ...
 * @param {string} latex - Contenu de l'environnement tabular.
 * @returns {string} - Contenu converti en HTML.
 */
function convertTabularToHTML(latex) {
  // Extraire le contenu entre \begin{tabular}{...} et \end{tabular}
  const tabularContent = latex.replace(/\\begin\{tabular\}\{[^}]*\}/, '').replace(/\\end\{tabular\}/, '').trim();

  // Séparer les lignes
  const lines = tabularContent.split('\\\\').map(line => line.trim()).filter(line => line.length > 0);

  // Convertir chaque ligne en cellule HTML
  const htmlRows = lines.map(line => {
    const cells = line.split('&').map(cell => cell.trim());
    const htmlCells = cells.map(cell => `<td>${cell}</td>`).join('');
...
/**
 * Fonction pour convertir un environnement 'itemize' en HTML.
 * @param {string} latex - Contenu de l'environnement itemize.
 * @returns {string} - Contenu converti en HTML.
 */
function convertItemizeToHTML(latex) {
  // Extraire le contenu entre \begin{itemize} et \end{itemize}
  const itemizeContent = latex.replace(/\\begin\{itemize\}/, '').replace(/\\end\{itemize\}/, '').trim();

  // Séparer les items
  const items = itemizeContent.split(/\\item\b/).map(item => item.trim()).filter(item => item.length > 0);

  // Convertir chaque item en liste HTML
...
/**
 * Fonction pour convertir un environnement 'enumerate' en HTML.
 * @param {string} latex - Contenu de l'environnement enumerate.
 * @returns {string} - Contenu converti en HTML.
 */
function convertEnumerateToHTML(latex) {
  // Extraire le contenu entre \begin{enumerate} et \end{enumerate}
  const enumerateContent = latex.replace(/\\begin\{enumerate\}/, '').replace(/\\end\{enumerate\}/, '').trim();

  // Séparer les items
  const items = enumerateContent.split(/\\item\b/).map(item => item.trim()).filter(item => item.length > 0);
...
/**
 * Fonction pour convertir des environnements spécifiques en HTML.
 * @param {string} content - Le contenu extrait.
 * @returns {string} - Le contenu avec les environnements convertis en HTML.
 */
function convertEnvironmentsToHTML(content) {
  // Conversion des environnements tabular
  content = content.replace(/\\begin\{tabular\}[^}]*\}([\s\S]*?)\\end\{tabular\}/g, (match, p1) => {
    return convertTabularToHTML(match);
  });

  // Conversion des environnements itemize
  content = content.replace(/\\begin\{itemize\}([\s\S]*?)\\end\{itemize\}/g, (match, p1) => {
    return convertItemizeToHTML(match);
  });

  // Conversion des environnements enumerate
  content = content.replace(/\\begin\{enumerate\}([\s\S]*?)\\end\{enumerate\}/g, (match, p1) => {
    return convertEnumerateToHTML(match);
  });
...
function main() {
  ...
  const commandsToExtract = [
    ...
    { name: 'date', jsonKey: 'date', isContent: false, isVerbatim: false },
