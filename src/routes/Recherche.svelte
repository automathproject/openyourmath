<script lang="ts">
  ...
  interface Exercise {
      uuid: string;
      titre: string;
      theme: string[]; // Modifié pour être un tableau de chaînes
  }
  ...
  onMount(async () => {
      try {
          ...
          if (response.ok) {
              ...
              // Extraire tous les thèmes uniques de tous les exercices
              themes = Array.from(new Set(
                  exercises.flatMap(ex => ex.theme)
              ));
  ...
  function applyFilter() {
      ...
      filteredExercises = selectedTheme === 'Tous les thèmes'
          ? exercises
          : exercises.filter(ex => ex.theme.includes(selectedTheme));
                      ...
                      >
                          <div class="exercise-info">
                              ...
                              <div class="exercise-theme">
                                  <small>{exercise.theme.join(', ')}</small>
                              </div>
                          </div>
                      </button>
                  {/each}
